...
int main(int argc, char** argv){
  ...
  
  {
    ...
    Parameters[2]  = input_file("live_ic",1.0);
    Parameters[3]  = input_file("dead_ic",1.0);
    Parameters[4]  = input_file("t_death",1.0);
    ...
    for(unsigned int j=0;j<size;j++){
      ...
      out_file << scientific << liveConfModel[0]+deadConfModel[0];
      for(unsigned int o=1; o<liveConfModel.size(); o++)
	    out_file << " " << liveConfModel[o]+deadConfModel[o];
